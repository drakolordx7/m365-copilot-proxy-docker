 ...
 */
/** True when `text` looks like a Cursor/M365 tool fence (with or without opening ```). */
export function looksLikeToolFenceChunk(text: string): boolean {
  const t = text.trim();
  if (!t) return false;
  if (/^```[A-Za-z][A-Za-z0-9_]*/.test(t)) return true;
  // Opening ``` often dropped when status prose + fence arrive as separate MessageUpdates
  return /^(ReadFile|Read|Glob|rg|Grep|Shell|Bash|Write|StrReplace|Delete|Subagent|CreatePlan|TodoWrite|ReadLints|WebSearch|WebFetch|AskQuestion|AwaitShell|Await)\b/.test(
    t,
  ) && /\n(?:path|glob_pattern|pattern|command|target_file|prompt|description)\s*:/i.test(t);
}

/**
 * Fold a token delta OR a full-text snapshot into `answer`.
 *
 * Extra Cursor/M365 case: status prose MessageUpdate then a separate tool-fence
 * MessageUpdate. The fence is not a prefix of the prose (and is often shorter),
 * so the old rules dropped it; later deltas appended `ReadFile…` without ```.
 * When `next` looks like a tool fence and isn't already in `answer`, append it.
 */
export function foldStreamText(
  ...
  if (next.length <= answer.length) {
    if (
      looksLikeToolFenceChunk(next) &&
      answer.length > 0 &&
      !answer.includes(next.trim())
    ) {
      const sep = !answer || answer.endsWith("\n") || next.startsWith("\n") ? "" : "\n";
      const chunk = sep + next;
      return { answer: answer + chunk, emit: chunk };
    }
    return { answer, emit: null };
  }
  if (next.startsWith(answer)) return { answer: next, emit: next.slice(answer.length) };
  // Divergent longer snapshot that is itself a fence — append rather than replace
  // so we keep the status sentence Cursor shows with tool_calls.
  if (looksLikeToolFenceChunk(next) && answer.length > 0 && !answer.includes(next.trim())) {
    const sep = answer.endsWith("\n") || next.startsWith("\n") ? "" : "\n";
    const chunk = sep + next;
    return { answer: answer + chunk, emit: chunk };
  }
  // Prefer keeping an answer that already has a fence opener over a divergent
  // rewrite that lost ``` (common when M365 republishes a mangled full text).
  if (/```[A-Za-z]/.test(answer) && !/```[A-Za-z]/.test(next) && looksLikeToolFenceChunk(next)) {
    return { answer, emit: null };
  }
            ...
            if (msgUpdate.success) {
              for (const m of msgUpdate.data.messages) {
                ...
                if (m.author === "bot" && m.text && !m.messageType) {
                  // foldStreamText appends tool-fence snapshots that diverge from
                  // prior status prose (Cursor/M365 often sends them separately).
                  advance(m.text);
                }
              }
              continue;
